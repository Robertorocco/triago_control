...
The rate term in~\eqref{eq:cost} penalises deviation from $\qdot^{\text{meas}}$, the \emph{measured} joint velocity, restricted to $\mathcal{A}$, anchoring the weakly-pinned wrist DOFs to the arm's real state every tick. It serves three purposes: suppressing oscillation in the measured joint velocity itself, suppressing the resulting visible oscillation of the physical robot, and acting as a soft limit on achievable joint acceleration. The term has two structural properties. First, the anchor must be the measured velocity, not the controller's own previous output $\qdot^{\star}_{k-1}$: the latter is a self-referential low-pass filter in the forward path of the closed loop, so it smooths the \emph{commanded} signal while pulling the command toward the controller's possibly-diverged internal history rather than the arm's real physical state. Second, the mask to $\mathcal{A}$ is mandatory: locked joints are already pinned to $\qdot_i = 0$ by two opposing rows of~\eqref{eq:jl}, and $\qdot^{\text{meas}}$ carries real motion and differentiation noise on exactly those joints, so an unmasked rate term would contest the pin and force two linearly dependent constraint rows into the active set simultaneously.
...
Blending is suspended whenever the commanded twist falls below the stillness thresholds, while the clutch is held for repositioning (Section~\ref{sec:device}), and throughout the autonomous grasp phases (Section~\ref{sec:grasp}). Assistance in this channel therefore \emph{modulates} the motion the operator is already making, never substituting for it.
...
\label{eq:kappacal}
\end{equation}
where $\kappa \in (0,1)$ is the deadzone-exit fraction and $\sigma_{\text{lin}}, \sigma_{\text{ang}} > 1$ are per-channel trims that let translation and rotation carry a different share of that exit force without changing the shape of the saturation. The constraint $\kappa\,\sigma_{(\cdot)} < 1$ keeps the guidance a strict \emph{bias}: as long as the product stays below unity, it can never push the handle out of the deadband on its own, so the operator remains the sole initiator of motion. Moving \emph{with} the guidance exits the deadband easily; opposing it means beating spring plus guidance. A product at or above unity would make the guidance a full-authority force the operator has to fight rather than a steer.
