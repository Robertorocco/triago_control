...
\label{eq:flip}
\end{equation}
and because $R_z(\pi)$ is its own inverse and transpose, the same map carries motion commands (device $\to$ robot) and rendered wrenches (robot $\to$ device). The device's rotational workspace is narrower than its linear one and unevenly ranged across its wrist axes, so the rotational range, not the linear one, is the binding constraint on the hand-to-gripper map; the home-orientation compression of~\eqref{eq:homerot} is what keeps a full gripper reorientation inside it.

Two terms apply to every rendered wrench regardless of condition. A constant viscous damper $(-K_d^{\text{lin}} v_h,\, -K_d^{\text{ang}} \omega_h)$, with $v_h$ and $\omega_h$ the handle linear and angular velocity, opposes handle motion. A damper is velocity feedback, and it is passive only when the opposing wrench is applied in the same tick in which the velocity is measured; it is therefore formed at the device driver, within the tick, and added to whichever wrench the active force law supplies. No force law of Section~\ref{sec:rendering} carries a damping term of its own, so viscosity enters exactly once, identically in every condition. A rate limiter then bounds the change of the commanded wrench between successive ticks,
\begin{equation}
...
\label{eq:slew}
\end{equation}
with $F_k, \tau_k$ the commanded force and torque at tick $k$ and $\dot F_{\max}, \dot\tau_{\max}$ the slew bounds. Bounding the increment as a vector lets a transition between force laws keep its direction rather than skew axis by axis as each component saturates. The limiter acts before the damper is added, so the damper's same-tick passivity is untouched. A rate limiter, not a low-pass filter, is the correct instrument: it is transparent to any signal changing more slowly than its bound, whereas a filter lags every genuine force (Section~\ref{sec:filtering}). The vibrotactile cues of Section~\ref{sec:columnbase}, a torque whose sign reverses every tick, are the fastest legitimate signal in the system, so the angular slew bound has a hard floor at the cue's own rate of change.
