...
The QP is solved with a dual active-set method (Goldfarb--Idnani~\cite{goldfarb1983}, \texttt{quadprog}). On the rare infeasible tick the commanded velocity is zeroed for that tick, a zeroed setpoint whose consequences are examined in the scope-of-validity discussion below.
...
\label{eq:slacksched}
\end{equation}
each arm carrying its own persistent LPF state. In free space ($\bar{\lambda}_X\to 0$) arm $X$'s slack weight rises to its ceiling $w_{\delta}^{\max}$ (tight tracking, expensive to relax); near an active constraint on that arm it drops toward its base $w_{\delta}^{\text{base}}$ (tracking yields cheaply, preserving safety authority) --- independently of the other arm. The output-stage filter is an empirical smoothing choice, not a proven stability requirement: it is observed on hardware to remove a sharp one-tick weight jump left by the input filter alone, closing a loop in which a higher weight presses the QP harder against the barrier and raises $\bar\lambda_X$, which the map returns as a lower weight; this loop is not formally shown to be stable, only observed empirically to behave well, a limitation worth flagging plainly here.
