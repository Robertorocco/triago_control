...
$\tau_{\lambda}$ & \texttt{SLACK\_FILTER\_TAU} & 0.15~s & shadow-price LPF \eqref{eq:lpf} \\
$\tau_{w}$ & \texttt{WEIGHT\_SLACK\_FILTER\_TAU} & 0.2~s & slack-weight LPF \eqref{eq:slacksched} \\
$\gamma_{\min}$ & \texttt{GAMMA\_MIN} & 0.7 & scheduled CLF rate floor \\
$\nu_{\min}$ & \texttt{sigma\_min} & 0.20 & time-scale floor \eqref{eq:timescale} \\
$k_{\nu}$ & \texttt{k\_lambda} & 3.0 & time-scale sensitivity \eqref{eq:timescale} \\
$\tau_{\nu}$ & \texttt{filter\_alpha} & 0.19~s & time-scale LPF (0.95 at 100~Hz) \eqref{eq:timescale} \\
\bottomrule
\end{tabularx}
...
$\tau_{\text{dir}}$ & \texttt{GOAL\_DIRECTION\_FILTER\_TAU} & 4.0~s & goal free-direction filter \eqref{eq:dirfilter} \\
$r_s$ & \texttt{SIDE\_STANDOFF\_FROM\_CENTER} & 0.03~m & side-grasp standoff \eqref{eq:sidegoal} \\
$d_{\text{app}}$ & \texttt{approach\_offset} & 0.05~m & top/placement standoff \eqref{eq:topgoal}, \eqref{eq:placegoal} \\
$m_{\text{cyl}}$ & --- & 0.02~m & side-grasp height clamp \eqref{eq:sidegoal} \\
$\epsilon_{\text{top}}$ & --- & 0.05 & top-roll degeneracy \eqref{eq:toproll} \\
$\rho_{\text{lo}}, \rho_{\text{hi}}$ & --- & 0.008 / 0.030~m & position-azimuth confidence \eqref{eq:sideaz} \\
$\eta_{\text{lo}}, \eta_{\text{hi}}$ & --- & 0.15 / 0.45 & heading-azimuth confidence \eqref{eq:sideaz} \\
$\epsilon_{\text{mix}}$ & --- & 0.15 & azimuth hold threshold \\
$\chi_0, \chi_{\text{lock}}$ & --- & 0.05 / 10~rad & finger-choice hysteresis \eqref{eq:sidehyst} \\
$s_{\text{lo}}, s_{\text{hi}}$ & --- & 0.12 / 0.30~m & hysteresis lock distances \eqref{eq:sidehyst} \\
$m_P$ & \texttt{PLATFORM\_PLACE\_MARGIN} & 0.03~m & placement rim margin \eqref{eq:placegoal} \\
...
--- & \texttt{CYL\_RADIUS\_INFLATION} & 0~m & optional radius inflation, unused in all reported trials \\
\bottomrule
\end{tabularx}
