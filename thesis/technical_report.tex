...
The manoeuvre was logged with the controller's own telemetry (Section~\ref{sec:sched}), reconstructed offline from the recorded rosbag, over a $25.1$~s trial: the shared quintic-timed reference for both arms runs open-loop from $t=0$ to $t\approx10$~s, marked by the dashed vertical line in every panel of Figure~\ref{fig:hwqpdata} below, after which the reference is held fixed at the crossed endpoint for the remainder of the recording. The physical robot sustained a mean loop rate of $126$~Hz with $15.7\%$ tick-to-tick jitter against the $150$~Hz nominal --- lower and noisier than in simulation, as expected on hardware --- without weakening the barrier itself, since $h_{\text{soft},X}$, $J_{\text{soft},X}$, and $d_{\text{safe},X}$ are recomputed from the current state on every tick with no filtering (Section~\ref{sec:cbf}). Low-level execution was faithful throughout: the root-mean-square mismatch between the commanded and the measured joint velocity, across all 14 arm joints, was $5.0\times10^{-3}$~rad/s, so the discrepancies reported below originate in the constrained optimum the QP is solving, not in how well the robot executed it.

\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{results/hw_3d_trajectory.pdf}
\caption{Commanded (dotted) and executed (solid) gripper path of each arm ($X \in \{R,L\}$, red/blue) over the home-to-crossed trial, in the robot base frame. A circle marks each path's start; a star marks the executed path's end and a diamond the commanded path's end, joined by a thin grey segment whenever the two differ. The black triad at each executed end pose is the gripper's own axes at the orientation actually reached. The faint floor traces are the executed paths' vertical projection, added as a depth cue.}
\label{fig:hw3d}
\end{figure}

Figure~\ref{fig:hw3d} shows what that conflict looks like in the workspace. Both executed paths visibly detour from a straight line to the target; the right arm's is the more convoluted of the two, consistent with its higher joint-limit engagement during the transit (Figure~\ref{fig:hwqpdata-a} below). Neither gripper fully closes on its commanded endpoint, and not by the same amount: the right arm's final gap is $0.42$~m in position but only $14.7^{\circ}$ in orientation, while the left arm's is $0.11$~m in position but $28.8^{\circ}$ in orientation --- each arm converges on a different coordinate of the same target pose, the residual falling wherever its own slack left it, rather than uniformly across position and orientation together.

\begin{figure}[t]
\centering
\begin{subfigure}[t]{0.48\linewidth}\centering\includegraphics[width=\linewidth]{results/hw_qp_colA.pdf}\caption{Safety filter}\label{fig:hwqpdata-a}\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}\centering\includegraphics[width=\linewidth]{results/hw_qp_colB.pdf}\caption{Adaptive weight and reference governor}\label{fig:hwqpdata-b}\end{subfigure}
\caption{Safety-filter and upstream-shaping telemetry over the same trial as Figure~\ref{fig:hw3d}, per arm ($X \in \{R,L\}$, red/blue); the dashed vertical line in every panel marks $t\approx10$~s, the end of the open-loop reference. (a), top to bottom: CBF shadow price $\lambda_{\text{cbf},X}$; joint-limit shadow price $\lambda_{\text{jl},X}$; CBF safety margin $h_{\text{soft},X}-d_{\text{safe},X}$ (dotted line at zero, the constraint boundary); raw minimum inter-geometry distance $d_{\min}$ (dotted line at zero); and the velocity-dependent safety margin $d_{\text{safe},X}$ of~\eqref{eq:dsafe} (dotted line at its static floor $d_0$). (b), top to bottom: the adaptively scheduled slack weight $w_{\delta,X}$ of~\eqref{eq:slacksched}; then the reference governor's raw (dashed) and governed (solid) linear speed $\|v\|$, angular speed $\|\omega\|$, position-error norm $\|e_p\|$, and orientation-error angle $\theta$ (Algorithm~\ref{alg:governor}, Section~\ref{sec:governor}).}
\label{fig:hwqpdata}
\end{figure}

Figure~\ref{fig:hwqpdata-a} is the safety filter's own account of the same conflict. Both collision shadow prices climb together during the transit; the left arm's spikes to the trial's peak of $557$ near $t=6$~s, and both arms show repeated smaller bursts of $100$--$150$ throughout the hold phase, each a fresh barrier engagement rather than one sustained event. The joint-limit shadow price is active mainly during the transit, then returns in two brief late bursts, around $19$~s for the right arm and $22$~s for the left. The bottom two rows report what those prices are protecting: the CBF safety margin dips to just below zero at its worst ($-2.6$~mm) and otherwise sits within a few millimetres of it for the whole hold phase, while the raw minimum inter-geometry distance floors at $4.2$~cm over the same stretch --- clearance is barrier-limited, the same signature later confirmed at study scale in Section~\ref{sec:ressafety}. The velocity-dependent margin $d_{\text{safe},X}$ of~\eqref{eq:dsafe} tracks each arm's own speed directly: it rises to as much as $5.3$~cm during the fast transit and relaxes back toward its $1.5$~cm static floor $d_0$ as each arm slows.

Figure~\ref{fig:hwqpdata-b} shows the two mechanisms that shape what the QP is even asked to do. The slack weight $w_{\delta,X}$ sits at its ceiling before either arm starts moving, collapses to its floor within the first few seconds of motion, and then cycles back toward the ceiling every time that arm's own shadow price relaxes --- the schedule of~\eqref{eq:slacksched}, now read directly from its own output rather than inferred from the slack it produces. Of the reference governor's four mechanisms (Algorithm~\ref{alg:governor}), only one is exercised by this manoeuvre: the commanded linear speed, angular speed, and orientation-error norm never approach their configured ceilings, so their raw and governed traces coincide throughout, visible as a single curve per row. Position-error bounding is the exception, and it is not a brief event: once the right arm's real pose falls behind its raw reference, the governed error is pinned at a common ceiling of about $0.30$~m on $78\%$ of the trial's ticks, and the left arm's error is clamped intermittently over the same stretch it is still converging. This is the same fact the shadow-price and slack panels already showed from the QP's side of the loop, seen here from upstream of it: whenever the safety filter is genuinely fighting the reference, the governor keeps re-issuing one the CLF can still make progress toward, rather than the raw, receding one.
...
The picture that table paints is the reason the non-parametric track is treated as primary here. Nine of the fourteen measures fail Shapiro--Wilk in at least one condition, several of them decisively, and five fail Mauchly as well. Minimum clearance is the extreme case on both counts, for a reason that is structural rather than statistical and is taken up in Section~\ref{sec:ressafety}. Consequently, the analysis of variance is reported for the structure it reveals --- whether an effect lives in the mode, in the assistance, or in their interaction, and how much variance it explains --- while every claim that one condition differs from another rests on the Holm-corrected rank tests drawn on the figures. Partial eta squared, written $\eta^2_p$, accompanies each effect as its magnitude. Four panels are drawn on a vertical axis clipped to the data range rather than starting at zero --- movement smoothness, minimum clearance, mean intent confidence, and the rendered force under the joystick mapping --- because in each of them the values occupy a narrow band far from zero and bars grown from zero would be indistinguishable. In those four the differences must be read against the axis and never as ratios of bar heights.
