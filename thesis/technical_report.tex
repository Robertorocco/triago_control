...
\vspace{0.3em}
\begin{subfigure}[t]{0.485\linewidth}\centering\includegraphics{results/hw_gov_v.pdf}\caption{Governor: linear speed}\label{fig:hw-govv}\end{subfigure}\hfill
\begin{subfigure}[t]{0.485\linewidth}\centering\includegraphics{results/hw_gov_w.pdf}\caption{Governor: angular speed}\label{fig:hw-govw}\end{subfigure}
\vspace{0.3em}
...
\caption{Joint-space view of the same trial: position and measured velocity of the seven joints of each arm, J1 (shoulder) to J7 (wrist). The dashed vertical line is the end of the open-loop reference.}
\label{fig:hwjoints}
\end{figure}

Figure~\ref{fig:hwjoints} is the same trial seen from the joints. No joint exceeds $0.27$~rad/s at any point, and every velocity trace is smooth: there is no trace of the high-frequency ripple that the unanchored cost had produced on this robot before the measured-velocity rate damping of Section~\ref{sec:rate} was introduced. The two arms do not stop when the reference does: the joints keep moving after $t\approx10$~s at up to $0.2$~rad/s, slowly and continuously, which is the settling that the shadow prices of Figures~\ref{fig:hw-lcbf} and~\ref{fig:hw-ljl} account for tick by tick --- the arms are still being moved by the barrier and joint-limit rows, not drifting. The two late joint-limit bursts are visible here as the last inflections of the position traces, around $19$~s on the right arm and $22$~s on the left, after which the configurations settle and the velocities decay to zero.
