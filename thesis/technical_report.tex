...
A second hardware test exercises the bimanual controller itself rather than perception. Each arm is driven open-loop, by the same quintic-timed Cartesian reference source used throughout controller development, from a common home configuration --- both grippers raised and held away from the body (Figure~\ref{fig:hwhome}) --- to the pose sampled from the other arm at the start of the motion, so the two arms swap sides and end up crossed in front of the torso (Figure~\ref{fig:hwcrossed}). The manoeuvre puts the self-collision and inter-arm rows of the safety filter (Section~\ref{sec:cbf}) under a large, simultaneous bimanual displacement that a single-arm test cannot exercise.
