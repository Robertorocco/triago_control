...
Twenty-four participants completed the full grid of six conditions on both scenarios, giving 288 analysed trials; a twenty-fifth participant served as a pilot and is excluded. Every quantity below is first reduced to one number per trial, then averaged over the two scenarios so that each participant contributes exactly one value per condition, and it is those participant-level values that enter every test and every figure. Both scenarios are pooled rather than analysed separately because each participant met both under every condition, so the scenario is balanced out of the comparisons; the shield scenario is the easier of the two, and pooling therefore sets the absolute level of each measure between the two difficulties rather than at either one.
