...
\midrule
\textsc{clutch} guided feedback  & \checkmark & --     & tether $+$ guidance bias & teleop \\
\textsc{clutch} guided blending  & --     & \checkmark & tether (blended reference) & blending layer \\
\textsc{clutch} full guidance    & \checkmark & \checkmark & tether $+$ guidance bias & blending layer \\
\addlinespace[2pt]
\textsc{joystick} guided feedback & \checkmark & --     & spring $+$ guidance bias & teleop \\
\textsc{joystick} guided blending & --     & \checkmark & spring & blending layer \\
\textsc{joystick} full guidance   & \checkmark & \checkmark & spring $+$ guidance bias & blending layer \\
\bottomrule
\end{tabular}
\caption{The six study conditions ($2$ control modes $\times$ $3$ assistance strategies). The guidance bias is~\eqref{eq:fguide}; the blending layer is the arbitration of Section~\ref{sec:blend}.}
\label{tab:cells}
\end{table}

Fairness across the matrix follows directly from the architecture of Chapter~\ref{ch:method}: the safety controller, belief estimation, goal manifolds, the grasp state machine, and every shared building block (homing, damping, vibrotactile cues, authority cap) are byte-identical across all six conditions. A condition therefore changes only the operator-input mapping and the rendered assistance, so a measured difference between two conditions can only be attributed to the manipulated factors.
...
Both scenes also carry decorative industrial fixtures --- a perimeter fence, an overhead gantry, cabinets, pallets, and the conveyors' own frames and rollers. None of these carry collision geometry, and none enter the safety controller's world model; they give the operator visual context and nothing more. The barrier's forward-invariance guarantee (Section~\ref{sec:cbf}) is stated over the \emph{declared} collision model, so a fixture that is drawn but not declared lies outside that guarantee by construction. Each declared model contains only the table, the two batteries, the scenario's own blocking structure --- the rack shelf with its supports, or the shield pane --- and one convex envelope per conveyor.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{rackworld.png}
\caption{The relay scenario, viewed from behind the robot in Gazebo. The storage-rack shelf spans the table directly above the two batteries, closing the overhead approach corridor and forcing every pick and every place at the table into a side grasp; the handover disk (yellow) sits on the table between them, and the conveyor station (green, right) is reachable only by the right arm, so the left arm stages its battery on the disk for the right arm to relay onward.}
\label{fig:rackworld}
\end{figure}

\textbf{Independent scenario.} The task is two independent, concurrent pick-and-place errands, one per arm. A thin safety shield --- a glass pane in a steel frame in the Gazebo scene, with only its plain box collision declared to the safety controller --- stands between the robot and the table, spanning its full width so that no lateral window is left; each arm must therefore reach \emph{over} the shield to work. Two conveyor units flank the table, mirror-symmetric about the robot's sagittal plane, each carrying its own delivery station. Each arm picks the battery on its own side and delivers it to the station on that side: the errands are symmetric, with no handover and nothing contested between the arms. Because the shield closes the low approach and leaves the overhead corridor open, every grasp here is a top grasp --- the complement of the relay scenario, where the shelf closes the overhead corridor and every grasp is a side grasp. Between them the two scenarios exercise both grasp manifolds of Section~\ref{sec:policies} and both the coordinated and the independent regimes of the bimanual controller.
...
Both scenarios are built to put every part of the architecture to work, not only the arm being driven. The handover in the relay scenario is a discrete, bimanual instance of the same gap Section~\ref{sec:locality} identifies for a single arm: each arm's policy is a memoryless local feedback toward its own goal, so which arm carries the battery and when the handoff happens is a decision no per-arm policy computes on its own, exactly the kind this architecture leaves to the operator rather than to a global planner. The head is put to the same kind of test: in the independent scenario the shield's collision box spans the full height from below the table top to above the batteries, so a battery is not visible from a head pose that does not look past the shield, and finding that view is itself part of the task. Belief estimation is stressed the same way: every graspable battery carries both a top and a side goal candidate regardless of which one the scenario's geometry actually admits (Section~\ref{sec:policies}), so the object-level belief of~\eqref{eq:objbelief} must resolve not only which object but which approach is intended, from poses that start close together whenever the operator's motion is still small or hesitant --- precisely the split its object-level sum is built to survive. The operator is in turn asked for large reorientations that exercise both mappings' own orientation-synchronisation machinery (Section~\ref{sec:clutchmode}, Section~\ref{sec:joystick}): a full turn from the home or last-placed orientation into a top- or side-grasp approach, and the reverse on placement. And the relay scenario's confined reach under the rack shelf --- table below, shelf above, support legs alongside --- is the cluttered configuration Section~\ref{sec:cbf} names directly as the case its barrier aggregation is sharpened for.
...
\caption{The independent scenario, viewed from behind the robot in Gazebo. A transparent safety shield spans the table's full width, closing the low approach and forcing every grasp into a top grasp; two mirrored conveyor stations (green) flank the table, each reachable only by the arm on its own side, so the two errands run independently with no handover.}
