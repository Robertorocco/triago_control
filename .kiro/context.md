...
│       └── matlab/*.m                      MATLAB loaders, group stats, reports (§15.6, §15.7)
...
- Posture/joint-limit avoidance via a repulsive potential field: `v_ref = -K_GRADIENT · dH/dp`, `H(p) = 1/(1-p)² + 1/(1+p)²` on normalized joint position `p = (q - center)/half_width` (clamped to ±`V_MAX_POSTURE`). ~0 at the center, explodes near a limit. Weighted by `W_CENTER`, scaled by a global `posture_scale` that ramps down to `POSTURE_GRASP_SCALE` during autonomous precision-grasp phases and back to 1.0 otherwise. `center` defaults to the range midpoint but may be retargeted per world (`POSTURE_TARGET_BY_WORLD`), in which case each side is normalized by its **own** half-width so `p` still reaches ±1 exactly at both real limits — the potential keeps diverging at both while its minimum sits off-midpoint. `_posture_indices` is therefore recomputed every tick rather than cached (cheap: <20 joints, no Jacobians).

**Wrist-branch escape** (`WRIST_BRANCH_MIN_BY_WORLD`, `shield_world`): `arm_*_6_joint`'s URDF range is asymmetric (`[-1.885, 3.002]`); below ~0 the arm sits in a **disconnected IK branch** whose whole self-motion manifold is jammed against that lower limit — unrecoverable without abandoning the task pose. The CLF task row is indifferent to which branch the joint occupies (both track the EE equally), so nothing else ever drives it back. Two mechanisms share one latch: a posture **recovery pull** (`POSTURE_TARGET_BY_WORLD` + `POSTURE_TARGET_WEIGHT_BY_WORLD`, weight ×10) active *only* until the joint crosses the floor, then a **permanently latched** joint-limit-CBF floor takes over while posture reverts to default. The floor is applied only after the joint has crossed above it under that pull — arming it against a startup pose already below it makes the CBF demand a recovery velocity past the joint's own `velocityLimit`, an inverted box that fails as "constraints are inconsistent".
...
**Head-as-obstacle**: the head chain (`arm_head_1..7_link`) is added to the arm QP's collision model as a quasi-static CBF obstacle via live FK capsules. No head joint enters the decision vector; the existing zero-lock on non-arm joints makes the barrier satisfiable only through arm motion. **This half is not sufficient on its own**: `d_safe_dynamic` thickens with the ARM's own speed only, so head motion is an unmodelled disturbance to it — a head moving at ~0.2 m/s crosses the ~7.5 cm usable gap (`DISTANCE_FILTER_THRESHOLD − CAPSULE_RADIUS − D_SAFE_BASE`) in ~375 ms and produced a real head-arm collision. The symmetric half lives head-side (§7.1), where the margin does see the head's own speed.
...
- **`get_object_belief`**: sums belief across a cylinder's grasp types (`goal_key.split('_')[0]`). The grasp-trigger gate uses this OBJECT-level mass, not the raw per-goal-key belief — two grasp types of the same cylinder (e.g. Top/Side) splitting posterior between themselves was blocking a confident, correct grasp on that object. `pos_costs`' 10% tie-breaker cannot separate them either: it's normalized by the spread across ALL goals, which the far ones set, so proximity to the near pair barely moves it — raising `POS_COST_WEIGHT` to compensate was tried and made belief converge slower, not faster (reverted).
...
`ALIGN_ALPHA_IDLE=0.35`, `ALIGN_ALPHA_MIN=0.2`, `ALIGN_ALPHA_MAX=0.8`; opposed motion yields fully at `s=-1`, so there is no permanent counter-pull.
...
- **`Side`**: the approach azimuth is a confidence-weighted mix of two estimates that fail in *disjoint* configurations — the horizontal anchor→axis radial (dead ON the axis, and noise-amplifying by `d/r` near it) and the gripper's own horizontal heading (dead only when the gripper points near-vertically, i.e. a Top posture). So a gripper hovering over the cylinder top still resolves a side from where the operator's wrist points, instead of holding an azimuth committed elsewhere. Ramping confidence replaces a hard radius: crossing a threshold re-committed the azimuth to whatever direction tremor was pushing, and near the axis that direction is noise — enough to walk the goal, and the guidance force with it, right around the cylinder. Only when both estimates are weak or they cancel is the last azimuth held. Plus the unchanged discrete fingers-up/down candidate on distance-scaled sticky hysteresis.
- **`Top`**: position pinned above the cylinder axis and the approach axis to world −Z, but the ROLL about that axis is a free manifold — a cylinder lifts identically at any roll — so the goal is the circle `R(θ) = Rz(θ)·R_TOP_DOWN`, resolved in closed form to the point closest to the anchor: `θ* = atan2(M01−M10, M00+M11)`, `M = R_TOP_DOWN·R_anchorᵀ`. Continuous in the anchor, so no discrete candidates and no hysteresis. Objective amplitude is `1−x_anchor_z`, vanishing only when the anchor points straight UP (the goal's antipode, every roll equidistant) — there the roll freezes, same remedy as Side's azimuth deadzone. **Consequence**: the Top goal's residual angular error is now EXACTLY the approach-axis tilt, so `ANG_ERR_ENTER/STAY` and the belief twist-cost stop charging the operator for a wrist roll that does not change the grasp (~34° of spurious error removed on a typical approach; ENTER becomes a clean "approach axis within 9.2° of straight down").
- **`Front`**: approach axis rigidly locked to world +X (reach/hover tutorial goals, no physical object).

Both free DOFs — Side azimuth and Top roll — are stored as unit 2-vectors and low-passed by a normalized EMA (`GOAL_DIRECTION_FILTER_TAU=0.15 s`; per-call gain derived from the owning node's `CONTROL_HZ` via `GoalSet(update_rate_hz=…)`, `0` disables). Normalizing the blend instead of averaging angles keeps it wrap-free, and the direction memories double as the filter state, so the value held while a signal is degenerate is the smoothed one. This bounds how fast a goal can travel around its own manifold: near a singularity the raw estimate still slews fast, and rendering that straight through is what reaches the operator as an oscillating force. `update_memory=False` calls return the committed state without advancing the filter, so visualization/lookahead anchors can neither spend a filter step nor observe a goal ahead of the tracked one. Note the normalized EMA converges slightly slower than `τ` for large steps (~55% at 1τ for a 90° step vs 63% linear) and approaches `τ` for small ones — a mild slew limit exactly where it is wanted.
...
**`RELEASE_LIFT` is distance-gated, not time-gated**: backs out `RELEASE_RETREAT_DISTANCE=0.14 m` along the reverse approach axis (blended with world +Z, same direction as `ABORT_RETREAT`) at `RELEASE_RETREAT_VELOCITY=0.05 m/s`, capped at `RELEASE_RETREAT_MAX_S=7 s`. Replaces a time-gated pure-vertical lift: a commanded velocity that the CBF/joint-limits/tracking lag silently eats leaves the barrier re-engaging with the gripper still overlapping the just-placed object (a SIDE placement especially — the fingers stay wrapped around the cylinder while rising), wedging the arm. Gating on measured EE travel instead of elapsed time guarantees actual clearance before handoff.

**Grasp confirmation is purely geometric** (no force/torque or vision sensing): signed gripper-box↔object `hppfcl` distance against the object's believed (kinematic) position/axis. Required overlap per grasp type (`_contact_depth_threshold`): `Top = -0.045 m` (shallow, vertical approach), `Side = -0.04 m` (deeper seating). Insertion depth per type (`_insertion_travel`, straight-line advance from the standoff — `approach_offset=0.05` in `goal_set.py` — along the approach axis): `Top = 0.09 m`, `Side = 0.068 m`. The two are coupled: a shallower insertion caps how deep contact can register, so cutting one requires relaxing the other or approach times out just short of the threshold. The `real-hw` branch tunes both deeper — diff against it before assuming either branch's numbers apply to the other.

**PRE_GRASP → GRASP_ALIGN trigger** (`_can_align`): asymmetric ENTER/STAY gate on `(pos_error, ang_error)` against the goal standoff — `POS_ERR_ENTER=0.040 m`/`ANG_ERR_ENTER=0.16 rad` to arm the transition, looser `POS_ERR_STAY=0.072 m`/`ANG_ERR_STAY=0.224 rad` to hold it once armed (avoids chattering at the boundary). `GRASP_ALIGN` itself then has to close the remainder autonomously within `ALIGN_TIMEOUT_S=12 s`, down to `ALIGN_POS_TOL=0.0209 m`/`ALIGN_ANG_TOL=0.103 rad`/`ALIGN_CENTERING_TOL=0.0207 m` — so ENTER sets the size of the autonomous search the FSM must still finish on its own (tightening ENTER shrinks that search, at the cost of making the operator work harder to trigger it in the first place).

**`/shared_autonomy/grasp_gate`** (`String`, 5 Hz, `main_shared_autonomy.py`): hysteresis-aware live telemetry of the FSM's own trigger state — current phase + `BLOCKED_BY=[...]` naming which gate (pose, belief, ...) is failing. Added to stop diagnosing trigger failures by inference from console spam; read this topic first before retuning any of the tolerances above.

## 6. World Scene Loading (`world_loader.py`, `config/worlds/*.yaml`)

Static obstacle layouts (table, graspable cylinders, walls, shields, etc.) described declaratively per scenario in `config/worlds/<name>.yaml`, not hard-coded. Schema: list of `ObstacleSpec` (name, role, shape, pose, size, color, collision on/off) + `grasp_roles: {red: <name>, blue: <name>}` + either a singular `platform:` (legacy, one placement disk) or a `platforms:` list (`handover_world`, `rack_world` — e.g. rack_world's `Handover` on the table + `Final` on the conveyor belt), all visual-only, never in the collision model. `PlatformSpec` entries are demandable by either arm once holding a cylinder; belief/guidance picks which one.

**`rack_world`**: storage rack shelf over the table forces SIDE grasps (blocks the top-grasp corridor); a CONVEYOR station on the robot's right (negative y) carries the `Final` placement, reachable only by the right arm, so the left arm must stage its cylinder on the `Handover` disk (`[0.86, -0.09]`, r=0.07, decentred right of the table centreline and 0.03 m clear of the red cylinder) for the right arm to relay onward — the handover is forced by reach, not a divider obstacle. The `.world`'s decorative industrial-cell scenery (fence, gantry, cabinets, pallets — all visual-only, no `<collision>`) is deliberately absent from the `.yaml`: `CollisionManager` never sees it, only `VisualizationEngine`/Gazebo render it. Must be manually copied to Gazebo's actual load path after editing: `cp config/worlds/rack_world.world ~/exchange/ros2-ws/src/pal-packages/pal_gazebo_worlds/worlds/rack_world.world` (plain copy, not a symlink in this workspace) — validate the XML parses (`python3 -c "import xml.dom.minidom as m; m.parse(path)"`) before copying, double-hyphens inside SDF comments break the parser silently otherwise. `rack_leg_front_center`'s YAML `color` alpha is 0.6 (translucent) rather than 1.0 like its siblings — `VisualizationEngine.publish_obstacle_marker` reads each obstacle's RViz marker color straight from this field, so it renders translucent identically in all three study RViz stations (no per-window override; they share one marker topic).
...
Teleop sibling of the visual servo (same independent-QP / IBVS-PBVS structure, `x = [dq_head (7), slack (3)]`). Assumes exactly one arm active at a time: subscribes to `/shared_autonomy/active_arm` (`std_msgs/String`) and points the camera at that arm's TCP, `gripper_{side}_grasping_link` (`cfg.RIGHT_TCP_FRAME`/`LEFT_TCP_FRAME` — the same frame the arm QP/belief estimator/grasp FSM treat as "the hand" everywhere else, not the wrist flange `arm_{side}_tool_link`) via FK. This node builds its own standalone Pinocchio model (no shared state with the arm QP), so it injects the frame itself on real hardware if the URDF lacks it — `_ensure_grasping_frame`, mirroring `robot_kinematics.py`'s identical injector (same measured offset from `gripper_{side}_base_link`), with a same-tick fallback to `arm_{side}_tool_link` if even that parent is missing.

Two objectives via one 2.5D IBVS equality task on the active hand: **centering** (pixel error → image centre, `J_task = L_s · J_cam`) and **stand-off** (depth error `Z − TARGET_DISTANCE`, default 0.7 m, `target_distance` ROS param). Falls back to PBVS look-at when the hand is off-FOV. FOV barriers (4 rows) + same joint-limit CBF buffers.

`JOINT_WEIGHTS` is **uniform**. The soft terms below enter the cost un-compensated (`H += w·JᵀJ`, `g += w·Jᵀv`), so their authority is divided by the weight of each joint they need; a proximal-to-distal ramp therefore prices the shoulder out of reach of every one of them, and the orbit below can never recruit the proximal joints it structurally requires. Uniform pricing also raises the PD floor `quadprog` needs on `H`.

**Velocity profile is shaped by the cap/gain pair**, not by either alone (`MAX_VELOCITY`/`-p max_velocity` = 0.20 rad/s, `LAMBDA_VISUAL`/`-p lambda_visual` = 2.5). Speed is `min(cap, λ·e)`: flat at the cap until the crossover `e = cap/λ` (0.08 rad), exponential below. The cap therefore owns the far field and λ owns the near field — lowering the cap while raising λ trades far-field speed for near-field speed, with a single crossover at `e = cap_old` where the two profiles meet (11.5° here: slower above, up to 2.5× faster below). Tuning either in isolation moves the whole curve, which is usually not what is wanted. Note the coupling to §3's **head-as-obstacle**: the arm QP treats the head chain as a *quasi-static* CBF obstacle and its `d_safe_dynamic` reflects only the ARM's own velocity, so head speed eats margin the arm cannot see (≈0.7/0.93/1.4 mm closure per 150 Hz tick at caps 0.15/0.20/0.30). The head's own CBF (below) is what actually covers this; watch `/qp_debug/min_distance` when raising the cap.

**Arm-switch homing** (`ENABLE_SWITCH_HOMING`/`-p switch_homing`, `-p home_pose`): an arm switch suspends the visual task and drives the head to a compact home configuration before re-acquiring, since re-entering tracking from a stretched chain is unreliable. Implemented as a third phase ahead of IBVS/PBVS that solves with **`meq=0`** (no equality rows at all — the cost target becomes a joint-space servo `K_HOME·(q_home − q)` instead of the posture+lean field, and the roll/approach-orbit terms are skipped as meaningless without a visual task). Both CBFs — joint-limit box and head-vs-arm — stay appended unconditionally, so the retract itself cannot drive the head into an arm. Ends on `HOME_TOL` (0.12 rad max-abs joint error) or `HOME_TIMEOUT_S` (2.5 s), whichever first; already-home switches cost ~nothing. `home_pose` defaults to `HOME_POSE_DEFAULT`, a neutral pose captured live and logged once at startup in copy-pasteable param form (`[Pose] Current head configuration -> -p home_pose:=...`, alongside the camera's Cartesian pose for reference only — the mechanism stays joint-space); an explicit empty override still falls back to joint mid-range. **Y-biased per side**: `home_pose_right`/`home_pose_left` are resolved once on the first ready tick by nudging that seed's camera world-Y by `±HOME_Y_BIAS` (0.20 m; right → more negative, matching `WORLD_RIGHT`) via `_bias_home_toward_y` — a bounded LOCAL correction (not general IK), reusing chain-lean's exact single-row damped-pseudo-inverse step, iterated to convergence from an already-good seed. `home_target` is selected live each tick by `self.active_arm`, so a double-switch mid-homing retargets without extra state. Choose a home CENTRAL to both arms' tracking poses: one far from where tracking leaves the chain pays close to the full timeout on every switch, and one near an arm makes the collision CBF fight the servo (tell: `[Homing] Complete` logging a residual well above `HOME_TOL`).

**Head-vs-arm collision CBF** (`ENABLE_HEAD_CBF`/`-p head_cbf`): the symmetric half of §3's head-as-obstacle. Reuses the arms' own barrier rather than a lookalike — `compute_softmin_jacobian` gained an optional trailing `idx_head=None` which, when supplied, publishes `(J, h_soft, d_safe, active)` on `CollisionManager.head_cbf`; the return tuple is unchanged so every arm-side caller (incl. `_real`'s async worker) is untouched, and with `idx_head=None` nothing is computed. This is nearly free because `J_dist_k = n·(J1−J2)` was always a full-`nv` row: the head columns existed all along, discarded only because the arm QP zero-locks non-arm joints. Constraint is the arms' form but deliberately NOT the arms' gains — `J_soft[head_v_idx]·dq ≥ −GAMMA_CBF_HEAD·(h_soft − (d_safe_dynamic_h + D_SAFE_HEAD_EXTRA))`, with `d_safe_dynamic_h` driven by the HEAD's own last commanded velocity (at 0.3 m/s it brakes ~200% earlier than static — the term §3's margin structurally lacks). `GAMMA_CBF_HEAD` (0.35) is half `cfg.GAMMA_CBF` (0.70) and `D_SAFE_HEAD_EXTRA` (0.03 m) stacks an extra fixed margin on the head side only — head brakes earlier and softer than the arm does for the identical encounter, by design: a close call should cost camera-tracking performance, not arm task performance. The arm's own barrier on this same pair (§3) is untouched. The head node builds its own `CollisionManager` and prunes to head-involving pairs (`define_collision_pairs` never pairs the head with the world, so scope is head↔arm); pruning is a cost optimization only, since the barrier routes by head membership either way. **This is the hard limit the soft approach-align pull resolves against** — expect `approach_err_deg` to hold a floor near the arm, which is correct. Watch for infeasible solves: added rows can empty the feasible set against the joint-limit box, and the `except ValueError` path commands zero velocity (head freezes rather than colliding — safe, but diagnose via the `h_head` in that warning and telemetry float 10).

**PBVS slack must be re-priced** (`W_SLACK_PBVS`): the three slack variables are pixel rates in IBVS but an ANGULAR VELOCITY (O(1) rad/s) in the PBVS branch, so the `W_SLACK_PIXELS`/`W_SLACK_DEPTH` pair would price look-at tracking *below* the posture field — the head settles mid-range and never acquires the hand, and in ~40% of configurations rotates AWAY from it (measured). All three components are one vector and share one weight in that branch. Any future cost term added to this QP has to be checked against BOTH branches' units.

**Posture = joint-limit repulsive field** (`K_POSTURE_GRADIENT`/`-p posture_gain`, `V_MAX_POSTURE` = half the velocity cap so posture can never claim the whole budget), the same `H(p) = 1/(1-p)² + 1/(1+p)²` formulation as the arm QP's field in `qp_formulator.py` — near-zero mid-range, diverging at a limit. This is what makes arm switching work from a stretched pose: the joint-limit CBF sets `upper_bound = min(MAX_VELOCITY, γ·(q_max − q − buf))`, so a joint parked near its limit is throttled toward zero velocity and the chain cannot swing across; the field keeps joints off their limits so that headroom survives. Field and CBF push the same direction near a limit (the CBF's box there admits only retreat velocities), so they cooperate rather than compete. Mid-range for the 7-DOF head arm is a bent-elbow pose, so this doubles as the "start non-stretched" bias — no nominal joint vector to hand-pick.

**Soft roll alignment**: keeps image-right (camera X) aligned with world-right (`WORLD_RIGHT = world -Y`) via signed roll error `theta = atan2(d_y, d_x)`, `d = R_cam^T · WORLD_RIGHT`, roll rate `wz = K_ROLL_ALIGN·theta`. Encoded as an LS term in the QP cost (`H[:7,:7] += W_ROLL_ALIGN·J_roll J_roll^T`, `J_roll = J_cam[5,:]`), not an equality row, so it only uses leftover redundancy and can't override tracking. Gated off while off-axis or near-gimbal.

**Soft approach-axis alignment** (`ENABLE_APPROACH_ALIGN`/`-p approach_align`, weight `-p approach_weight`): aligns optical axis (+Z) with the gripper's approach axis — `gripper_{side}_grasping_link`'s local +X, the same TCP convention `goal_set.py` uses for grasp-goal orientations (top grasp → top-down view) — a position/orbit bias, not an orientation servo, since centering owns axis direction. `p_cam_des = p_hand - TARGET_DISTANCE·x_orbit`; LS term on linear velocity toward it (`Jlin = J_cam[:3,:]`), weighted 10× `JOINT_WEIGHTS` so it actually wins the redundancy left after centering+stand-off. Still soft and centering-gated (fades to zero by `APPROACH_GATE_ANGLE_DEG`), so it can never fight PBVS acquisition nor override the hard equality/FOV rows. `x_orbit` blends the raw approach axis toward world -Z by `-p top_view_bias` (default 0.3): a no-op for Top grasps (axis already ~down) but lifts a Side grasp's flush, eye-level viewpoint toward overhead without discarding the "look along the reach axis" framing. `approach_err_deg` telemetry stays the raw, unbiased angle to the true approach axis (independent of the bias) — alignment is only achievable where that line is reachable (e.g. a gripper pointing up would need a camera below the hand), so the QP gets as close as it can and that residual shows up there.

**Soft chain lean** (`ENABLE_CHAIN_LEAN`/`-p chain_lean`): centering alone is cheapest to serve with the wrist (joints 5-7 own camera orientation outright), leaving the chain parked where the previous arm left it and the newly-active hand framed at a grazing, self-occluded angle. Lateral target on the head **elbow** `arm_head_4_link`: `y_elbow → LEAN_FRACTION·y_hand`. Elbow chosen because joints 5-7 are downstream of it → its Jacobian columns there are structurally zero, so the wrist *cannot* absorb the task. The single `LOCAL_WORLD_ALIGNED` row is damped-inverted into a joint velocity and added to the posture channel, which `g` pre-multiplies by `JOINT_WEIGHTS` (cost = `0.5‖dq − dq_target‖²_W`), so `K_LEAN` is a true 1/s independent of the joint weights.

Matplotlib plotting thread (guarded so a missing display never stalls control, `-p plot:=false` to disable) shows centering/roll/approach error + stand-off distance. The per-tick `[DEBUG]` console dump (T_hand/T_cam/P_cam/dist/ang_err/lean_err/dq) is opt-in via `-p debug:=true` (default off — at 1 Hz it drowned every other console message). Telemetry on `/head_active_tracking/{error,qdot,cartesian_cmd}`. Run: `ros2 run triago_control head_active_arm_tracking.py`.
...
**Full simulation launch sequence** (robot side, then teleoperation side). `simulation_bringup.launch.py` (added, replaces the old 4-terminal Gazebo/controllers/RViz sequence) starts Gazebo + robot spawn, TSID default controllers (`CONTROLLER_DELAY_S=10s` after spawn, controller_manager isn't up before that), the mobile-base teleop, and THREE RViz stations at `RVIZ_DELAY_S=14s`: `study_operator.rviz` (head camera + left-arm FPS tracking view), `study_overview.rviz` (static view zoomed on the table where the graspable objects sit — despite its filename, not a wide shot; saved `Orbit` `Distance≈0.98`), `study_right_tracking.rviz` (right-arm FPS tracking view, a clone of the operator config differing only in its current view and window X). Together the two FPS-tracking views keep the operator's active-arm reference in at least one view's FOV at all times. The operator's actual wide, whole-scene view is Gazebo's own GUI window (gzclient) — a fourth, non-RViz view, framed from behind the robot to match the device-to-robot frame convention (180° yaw flip, §9) so no side/top perspective disorients a first-time participant. Five views run simultaneously in total: gzclient (whole scene) + head camera image + the three RViz windows above. Window placement lives in each file's `Window Geometry` block. It also starts `study_robot_model_filter.py` (`ROBOT_MODEL_FILTER_DELAY_S=3s`, `dim_mode` launch arg — `material` default / `remove`), which republishes `robot_description` on `/robot_description_filtered` with every link outside the two arm/gripper chains either dimmed (translucent grey, each link given its own uniquely-named material so it can't collide with a kept link's original material) or stripped outright — all three RViz configs point their RobotModel display's Description Topic there instead of the real `/robot_description`. This exists because RViz's own per-link Alpha/Value overrides do not render for this robot's Collada meshes (confirmed both via a saved `.rviz` edit and a live GUI change — neither took visual effect, a suspected Ogre material-sharing limitation, not a config bug), so the filter changes geometry/material at the URDF source instead of trying to override it live in RViz; `<collision>`/`<inertial>`/joints are untouched, and the real `/robot_description` topic is never touched (other consumers need the complete tree). Republishes every 1s for `REPUBLISH_WINDOW_S=20s` then stops (not forever — a continuous republish makes RViz tear down and rebuild its whole Links property tree on every message, collapsing any branch the operator has expanded). It does NOT start the QP/shared-autonomy nodes or `study_recorder.py` — those stay separate terminals so they can be restarted independently without relaunching Gazebo:
```bash
cd ~/exchange/ros2-ws
...
#   args: end_effector_right/left (default pal-pro-gripper), base_controller,
#   rviz_stations (both default true) -- world_name must match on the nodes below.
...
- Diagnosing a controller regression: change **one** `config.py` gain at a time, state every implicated value from a fresh read of the file (values get edited live, also by parallel sessions), and confirm with the plotter's Task Authority / Posture Weight panels which term actually moved — several adaptive terms produce the same oscillation-like symptoms.
...
`export_to_matlab.py` (needs a sourced ROS env, like `study_metrics.load_bag`) exports every `OK` trial to `<DATA_ROOT>/matlab_export/`: one `mat/<participant>_<world>_<cell>.mat` (`meta` + per-arm `compute_metrics()` + every `series.<topic>` `load_bag` decodes — no raw `/joint_states`/`/tf`, same scope as `analyze_trial.py`'s dashboards) plus `manifest.{csv,mat}` (one row per trial, metrics flattened `right_*`/`left_*` — the whole-study entry point). Struct fields are sanitized to valid MATLAB identifiers (≤63 chars) and `savemat(..., long_field_names=True, oned_as='column')` is mandatory — several real topic names exceed the default 31-char struct-field limit and fail to save without it. `--skip-existing` reuses a `.mat` newer than its bag's `metadata.yaml` (manifest row read back from it, bag not re-decoded) — adding one participant costs ~2-3 min, not a full re-read of the corpus. `scripts/analysis/matlab/{load_manifest,load_trial,load_trial_row}.m` are the MATLAB-side counterparts, verified against real MATLAB (not just `scipy.io` round-trips). `make_matlab_bundle.sh` ships `matlab/*.m` + manifest + `participant_schedule.csv` + `prebuilt/` (PDF/HTML of the latest run, readable without MATLAB) as a self-contained folder and zip.

### 15.7 Group Statistics & Report (`scripts/analysis/matlab/`, needs Statistics and Machine Learning Toolbox)

`run_study_analysis.m` answers, per metric family and per metric: Q1 mode (paired t + Wilcoxon), Q2 assistance (RM-ANOVA GG + Friedman + Holm pairs), Q3 six cells (2-way RM-ANOVA + Friedman), Q4/Q5 consistency (between-participant SD, Pitman–Morgan paired-variance test, Kendall W), Q6 learning/order (per-participant slope over schedule slots; C-first vs J-first check), QW world — writing `analysis_results/study_results_<date>_n<N>/` (`results.mat`, CSVs, `figures/`). `build_report.m` converts `study_report.m` (publish markup, full metric/test math) into an executed `study_report.mlx` + HTML/PDF; figure capture works in `matlab -batch` too, but only for figures created without an explicit `Visible` argument (passing even `'Visible','on'` drops them from the output). `build_paper_figures.m`/`study_paper_figures.m` are the short counterpart: paper-style panels (one quantity per panel, the six cells, SEM whiskers, Holm-corrected p-value brackets) with every figure explained in plain language and followed by a written reading of the final data, exported `HideCode` so the PDF reads as a document. **Data collection is complete**: 24 participants P01–P24, 288 trials, P00 = pilot (excluded; not in the schedule); commentary in `study_paper_figures.m` is computed live from `S`/`R`, not hard-coded, so it stays correct for whichever run it's built against. Known data facts the commentary covers: the barrier multiplier is heavy-tailed in ~1 trial in 8 (spikes 1e3–8e6 during real barrier fights, P18 rack JF the largest) so the analysis compares `cbf_lambda_active_median` (median λ over active samples) and keeps `cbf_lambda_mean` diagnostic only; time-based mode comparisons carry an order/practice confound (task time −11.5 s per slot) while the composite, safety, motion and intent scores do not; "fraction of trial" metrics (`cbf_active_frac`, `safety_nearmiss_frac`) invert the assistance ranking versus their seconds (`cbf_active_s`, `safety_nearmiss_s`, derived) because blending removes free-space time — the doc shows both. **Clearance metrics** (`safety_min_dist_m`, `safety_mean_dist_m`, `safety_nearmiss_*`) are computed per arm from `/qp_debug/safety_margin + /qp_debug/d_safe_dynamic` (the barrier's own SoftMin `h_soft`, carried cylinder and bypassed target excluded, 1.0 sentinel → capped at 0.15 m): the raw `/qp_debug/min_distance` reads −3.5 cm whenever a cylinder is held (payload overlaps the gripper envelope) and is kept only as `safety_min_dist_graspincl_m`. Minimum clearance is ≈2 cm in every cell — the barrier's own limit, not an operator effect. `belief_mean_prob` (time-average of the leading goal's probability) replaces the saturated `belief_max_prob` as the discriminating intent metric. `export_to_matlab.py --recompute-metrics` re-runs `compute_metrics` from the series stored in each `.mat` (≈3 s/trial, no bag decoding) — use it after any `study_metrics.py` change. `run_study_analysis.m` also stores six-cell results for `DIAG_Q3` diagnostics (no family/composite row) so the doc can draw them; `fig_effect_map.m` (partial η² bars per metric × factor) replaces the ANOVA tables in the short document. `fig_paper_panels.m` draws them (bars from zero unless that flattens the panel → clipped axis, labelled; brackets skyline-packed, their vertical step solved for a fixed PIXEL gap so stacked p-labels cannot touch whatever the data span is; the axis floor *is* the bar base, with `ShowBaseLine` off, because padding under it renders as two parallel bottom lines). `studyplot.color` is one blue and one warm ramp that rotate hue as well as lightness: three steps of a single hue cannot hold the ≥15 OKLab ΔE that neighbouring bars need, and a dark low-chroma orange reads brown in print — re-check any palette edit with the OKLab/CVD checks rather than by eye. `study_summary.m` (called by the driver) writes the short publication-style output `summary/` (scorecard, forest plots with Holm-corrected p, families×cells matrix, validity checks, `summary.txt` verdict, `summary_table.csv`) — the deliverable that stays valid at any N. Task success / incident notes are deliberately NOT analysed (recorded inconsistently; the study targets assistance toward the goal poses, not placement precision). Design rules baked into `study_metric_spec.m`: `load_study_table` keeps only participants present in the schedule with all 12 trials (no hard-coded IDs); force/clutch metrics are never compared across modes; blending metrics only in B/FB cells; slots are averaged over worlds before any learning test because shield always preceded rack within a slot. Hand-written stats helpers were verified against scipy.

**Thesis figures** (`fig_thesis_panel.m`, `fig_thesis_learning.m` → `thesis/figures/results/*.pdf`): the same bars as `fig_paper_panels` but stripped for print — no title, no subtitle, no y label (the letter, quantity and unit live in the LaTeX caption), sized in centimetres so LaTeX includes them at native size, vector PDF. A metric not comparable across modes is drawn per mode from `Q2_withinC`/`Q2_withinJ` with `cell_colors` so the bars still carry that mode's ramp. Assumption checks for the thesis table (Shapiro–Wilk via scipy, Mauchly and Greenhouse–Geisser from the orthonormal contrast covariance) are computed outside MATLAB and were verified against MATLAB's own `mauchly()` to four decimals; most measures fail normality, so the Holm-corrected rank tests — not the ANOVA — carry every pairwise claim.

**Hardware-trial figures (thesis §4.1)**: `scripts/analysis/export_offline_bag.py <bag_dir>` reads an `offline_plotter.py` trial bag (or any sqlite3 rosbag2 of the QP stack) through sqlite3 + `rclpy.serialization` — no `rosbag2_py` needed — into one `.mat` (`series.<topic>.{t,data}`, JointState as `names/position/velocity`, `derived.gov`/`derived.traj` = offline_plotter's governor and tracking derivations replayed event-ordered). t0 = trigger rising edge, else falling edge − the `t_off` parsed from the trial's `trial_summary.txt` (the recorder is spawned from inside the trigger callback, so the rising edge is routinely missing from the bag), else first message. `scripts/analysis/matlab/fig_thesis_hw.m(mat, out_dir, 'panels', …)` draws one `hw_<key>.pdf` per panel (registry-based: λ_cbf, λ_jl, margin, d_min, d_safe, w_δ, governor raw/governed ×4, q/q̇ per arm, …; absent topics are skipped, so the same call serves a teleoperation bag), in `fig_thesis_panel` style; LaTeX lays them out as lettered subfigures. The raw bag used is in `thesis/figures/results/Home-> Cross2/` (untracked, 185 MB). `fig_thesis_hw.m`'s broken-axis panels and its `gov_ep`/`gov_theta` reference lines are trial-agnostic: any panel key can go in `'broken_panels', [...]` with a matching row in `'broken_ranges'` (`[bottom_ceil top_floor top_ceil]`, chosen per-trial from where the data actually has a gap — home-to-crossed λ_cbf: `[180 500 600]`, teleop λ_cbf: `[500 1000 1100]`, teleop λ_jl: `[16 24 28]`), and the governor ceilings (`0.30` m, `1.0` rad) are fixed real-hw-branch config constants, not read off each trial's own data (a trial whose governor barely saturates would otherwise draw a "ceiling" at its own coincidental peak). Teleop-only panel keys: `hap_{pos,rpy,vlin,vang,force,torque}` (Haption handle; the operator-in-control windows — deadman held AND clutch released, offline_plotter's `_control_intervals` — are shaded light yellow behind every `hap_*` panel; x/y/z key is the LaTeX `\hwxyzlegend`) and `sa_{alpha,share,belief,state}` (blend authority, policy share of the blended action — the user share is its complement and is not drawn —, per-goal belief in red/blue families with dark=top, light=side, and the active-arm band + grasp-active step). `on_gov` in `export_offline_bag.py` NaN-fills a side independently rather than skipping the whole tick when only one arm's `/arm_*/cartesian_reference` exists yet — a teleoperated trial's inactive arm publishes no reference at all until the operator switches to it (unlike home-to-crossed, where both start at t=0), and the old both-sides-ready gate silently dropped the *active* side's real governor data for as long as the other one was idle.

§4.1.2 reuses this pipeline on `thesis/figures/results/Real_Teleop_10_CFB_SUCCESS/` (untracked bag + `Teleop1-4.jpeg`/`Teleop*_crop.jpeg` + `PoseEstimation.png`, a *different* trial's perception-debug screenshot the user supplied as illustrative reference only): a CFB teleoperated bimanual grasp with one failed side-grasp (12.0 s alignment timeout), one successful top-grasp, and a failed second side-grasp whose retreat stalled and oscillated into an emergency stop (deadman release at t=217.3s) — timeline cross-validated from `/shared_autonomy/{gripper_cmd,grasp_margin,grasp_active,active_arm}` against `grasp_state_machine.py`'s own timeout constants. All §4.1.2 panels are truncated to `t_max=230` (a few seconds past the deadman release): nothing recorded after the operator releases the deadman reflects the controlled system, so the trailing ~23s of the 253.7s recording is dropped from every plot rather than dominating the axis with a meaningless flat tail.

**World source for real-hardware runs (verified against the `feat/real-hw-async-cbf`/`origin/real-hw` branch, commit `9fb8fb3`, the state active on 2026-07-17 when both hw trials were captured — read via `git show`, never checked out into this working tree, per the user's standing instruction to reason from that branch's code without pulling it here):** a real hardware run with vision does **not** set `world_source:=perceived` on `main_qp_controller.py`. Instead it launches an entirely different entrypoint, `main_qp_controller_perceived.py` — `class PerceivedQPController(RealQPController)`, i.e. a subclass of the real-hardware async-CBF controller (`main_qp_controller_real.py`) that overrides only `_build_world_scene()` to block at startup on the head's latched `/perceived_world/snapshot` (`world_name` is ignored entirely in this mode, no YAML fallback exists in the file at all). So the collision world for the teleop trial's two cups genuinely was the live RANSAC estimate, not a declared/configured YAML — my earlier note here calling it "configured, not estimated" was wrong, based on checking only `main_shared_autonomy.py`'s *default* parameter value rather than what was actually launched. The snapshot topic is still absent from `bag_0.db3` for an unrelated, already-familiar reason: it is published once, latched, at head-convergence — before this session's `ros2 bag record` (started late, from inside the trigger callback, same as the missing rising-edge trigger message elsewhere in this pipeline) began capturing. Recovering the real per-trial radius/height/pose therefore needs either a separate bag/log spanning the scouting phase itself, or the "one final QP-payload block" `main_head.py` prints to console on convergence (context.md line ~214 on that branch), if that console output was saved anywhere.

### 15.8 Handover — resuming the analysis on another machine

**In git** (both repos on `feature/sim-user-study`, pushed): all code, `scripts/analysis/participant_schedule.csv`, `thesis/`, this file. **Not in git** — `~/exchange/triago_study_data/` (44 GB), to be copied by hand:
- `P00`–`P24/` raw rosbags (~35 GB) — the only irreplaceable thing; archive them.
- `matlab_export/mat/` (8.4 GB) — per-trial `.mat` (metrics + decoded series). Regenerable from the bags in ~1 h *with a sourced ROS env*; copy it if the new machine has no ROS/Docker.
- `matlab_export/manifest.{mat,csv}`, `matlab_export/analysis_results/` (50 MB; `latest.txt` names the current run), `trials_summary.csv`.
- `triago_matlab_bundle.zip` (1.4 MB) — scripts + manifest + prebuilt PDFs: enough to redo the whole group analysis with MATLAB alone, no ROS, no Python.
- `User Study Questionnaire.csv` (50 KB, Google Forms export, currently `~/exchange/`) — the only source for §15.9; small enough to commit, but it carries age, gender and free text, so that is the user's call.

**Minimum to continue**: the bundle zip (group statistics and every figure); add `mat/` for anything needing per-trial time series (`load_trial`, new metrics). **Full**: the whole `triago_study_data/` at `~/exchange/triago_study_data/` or `TRIAGO_STUDY_DATA_ROOT`.

**Pipeline order** (each step idempotent): `check_study_data.py` → `export_to_matlab.py` (`--skip-existing` for new bags, `--recompute-metrics` after a `study_metrics.py` change; run with `python3 -s` if a user-site numpy 2.x shadows the apt scipy) → MATLAB `run_study_analysis` → `build_paper_figures` (→ `build_report` for the long one) → `make_matlab_bundle.sh`. `START_HERE.m` reopens the latest run. **For the thesis** the two remaining steps are MATLAB `build_thesis_figures` (16 objective panels → `thesis/figures/results/`) and `python3 scripts/analysis/subjective/run_subjective_report.py` (the questionnaire PDF, plus the 7 subjective panels into the same folder); then `latexmk -pdf technical_report.tex` in `thesis/`. Both figure steps overwrite in place and are idempotent, so the thesis recompiles unchanged after a full regeneration.

**Driving MATLAB from Claude**: `pip install --user matlabengine` pinned to the local MATLAB release (e.g. `25.1.2` for R2025a — check with `pip show matlabengine`, not the version elsewhere in this file; engine lives in user site, so *not* under `python3 -s`); in MATLAB `matlab.engine.shareEngine('TRIAGO')` once per session, then `matlab.engine.connect_matlab('TRIAGO')` from Python — figures open in the user's own window. `eng.run(scriptPath)` temporarily `cd`s MATLAB into the script's own folder to execute it, which breaks any pwd-based auto-detect (e.g. `study_export_dir`'s candidate 2) inside that script — prefer calling functions with an explicit `results_dir`/`export_dir` argument, or `eng.eval`, over `eng.run` for anything that resolves paths at runtime. Headless alternative: `matlab -batch "run('/tmp/x.m')"` (write a temp `.m`; a multi-line `-batch` string fails).

**Stale-export gotcha**: a `matlab_export/` copied before a `study_metrics.py` metric change (e.g. the 2026-09-16 clearance fix, commit `1c7b26d`) silently keeps the OLD values — nothing errors; e.g. `safety_min_dist_m` reads as a negative raw distance instead of the ~2 cm clearance documented in §15.7. Fixing it needs no raw bags: `export_to_matlab.py --recompute-metrics`'s outer loop normally sources `metadata` from each trial's raw bag folder via `--data-root`, which a `matlab_export/`-only machine doesn't have — instead read `meta` back out of each `.mat` (`metadata = _struct_to_dict(loadmat(path, struct_as_record=False, squeeze_me=True)['meta'])`) and call `recompute_trial_mat(mat_path, metadata)` per file (both from `export_to_matlab.py`), then `write_manifest`. Also rewrite `analysis_results/latest.txt` after copying — it holds the exporting machine's absolute path and breaks `study_summary`/`build_paper_figures`/`START_HERE` auto-detect elsewhere until repointed to the local path.

**State at handover (2026-09-17)**: data collection complete; canonical run is `study_results_20260917_112426_n24` (corrected clearance/safety metrics, restyled figures, and the previously-undrawn metrics added — supersedes every earlier run; `study_paper_figures.pdf` is now 27 pages and is the artefact the thesis results chapter is written from). `intervention_{mean,peak}_mps` = ‖v_blend − v_user‖ while the operator drives, from `blend_debug`, is the shared-control literature's **transparency** measure — it was missing entirely and is family-less so it changes no existing score. Force/clutch metrics are now drawn *within* each mode (`fig_paper_panels(..., 'source', "Q2_withinC"/"Q2_withinJ")`), which is the only valid framing for a quantity that is not mode-comparable. The seeded bootstrap (`rng(12345)` in `stat_paired2`/`stat_consistency`) makes a re-run reproduce every number exactly, so re-running only to restyle figures is safe. Open items: the questionnaire results (§15.9) still have to be written into the thesis; **the `human_effort` family score is `qdot_cmd_rms` alone** — `composite_score` admits only `mode_comparable` + `cell_scope=="all"` metrics, so force/clutch (the operator-side effort) sit outside every family score and outside the paper document, and "Human effort: C vs J not separated" is a statement about joint rates, not about the hand; `cbf_active_frac`/`cbf_lambda_*` are computed over the whole trial including autonomous-grasp windows while the clearance metrics exclude them; success/incident notes stay unanalysed by decision.

### 15.9 Subjective Analysis (`scripts/analysis/subjective/`, Python + pdflatex, no MATLAB)
`run_subjective_report.py --csv <export>.csv --results <study_results_dir>` reads the Google Forms export together with `trial_table.csv` and writes `<results>/subjective/subjective_report.pdf`. The form's six 1–7 items (mental demand, physical demand, perceived success, control, trust, comfort) sit in **six repeated seven-column blocks**, one per condition in the order CF, CB, CFB, JF, JB, JFB — the `Condition just completed` column selects the block, and the first block is also filled by the opening submission, so rows cannot be split on submission type. Data are complete: 24×6 reports, no missing rating, 48 valid ranking triples. Demand items are reversed (8−x) and averaged with the other four into a *subjective quality index* (Cronbach α = 0.85). Everything is rank-based (Friedman, Holm-corrected Wilcoxon, Mann–Whitney, Spearman); subjective↔objective links use a **repeated-measures correlation** (within-participant centring, df = N−k−1) so between-participant differences cannot inflate them. Findings: assistance dominates every item (W = 0.36–0.50, F worst everywhere, B vs FB null throughout), mode is subjectively invisible on all seven measures and in the forced choice (13 C / 11 J, p = 0.84) although the joystick wins objectively — and a participant's own objective advantage does not predict their choice (13/24, chance). Self-rating is calibrated *within* a participant (r = +0.32 vs composite, +0.50 vs task time) and not *between* them (ρ = +0.20, n.s.). Trust tracks filter activity (r = +0.45) not real clearance (r = +0.10); felt control is uncorrelated with how much authority the arbitration held; physical demand tracks re-indexing count, not rendered force or `qdot_cmd_rms` — corroborating from the operator side the §15.7 caveat that the human-effort family is a robot statement. Only 3 of 6 NASA-TLX dimensions were asked, so no TLX workload score exists; the report's caveat section lists the other nine limits. **Item polarity is an inference, not a documented fact**: a Forms CSV carries no scale anchors, and item 3 is named "Performance" in NASA-TLX's own numbering, where that dimension is anchored Perfect→Failure, i.e. reversed — it is scored here as high = more successful because the within-participant correlation with the composite (+0.32) and with shorter task time (+0.50) would have to flip sign otherwise. `thesis_figs.py` draws the thesis-styled subset into `thesis/figures/results/res_subj_*.pdf` (matplotlib with Nimbus Sans + Computer Modern mathtext, which matches the MATLAB panels' Helvetica + LaTeX interpreter); ratings symbols are $S_{md}$, $S_{pd}$, $S_{pf}$, $S_{ct}$, $S_{tr}$, $S_{sm}$ and the index $\bar{S}$, Kendall's W is written $W_K$ because plain $W$ is already the belief-cost metric. Panels draw only the two contrasts against F, so all six stack to two bracket levels; the B-vs-FB contrast lives in the table, where the one significant instance (physical demand, within clutch, p = 0.026) is noted in the text instead.
