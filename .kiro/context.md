...
**Pipeline order** (each step idempotent): `check_study_data.py` → `export_to_matlab.py` (`--skip-existing` for new bags, `--recompute-metrics` after a `study_metrics.py` change; run with `python3 -s` if a user-site numpy 2.x shadows the apt scipy) → MATLAB `run_study_analysis` → `build_paper_figures` (→ `build_report` for the long one) → `make_matlab_bundle.sh`. `START_HERE.m` reopens the latest run.

**Driving MATLAB from Claude**: `pip install --user matlabengine==24.1.4` (must match the MATLAB release, R2024a; the engine lives in the user site, so *not* under `python3 -s`); in MATLAB `matlab.engine.shareEngine('TRIAGO')` (once per MATLAB session), then `matlab.engine.connect_matlab('TRIAGO')` from Python — figures then open in the user's own window. Headless alternative: `matlab -batch "run('/tmp/x.m')"` (a multi-line `-batch` string fails; write a temp `.m`).

**State at handover (2026-09-16)**: data collection complete, final n=24 run built (24-page `study_paper_figures.pdf`, with the methods/assumption section 1b and per-figure readings). Open items: post-trial questionnaire (workload/trust/preference) is neither exported nor analysed; `cbf_active_frac`/`cbf_lambda_*` are computed over the whole trial including autonomous-grasp windows while the clearance metrics exclude them (one-line change in `study_metrics.py` + `--recompute-metrics`); success/incident notes stay unanalysed by decision.
