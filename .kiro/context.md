...
**Soft chain lean** (`ENABLE_CHAIN_LEAN`/`-p chain_lean`): joints 5-7 are a spherical wrist owning camera orientation and priced ~50x below the shoulder by `JOINT_WEIGHTS`, so centering alone is served wrist-only and the chain stays parked where the previous arm left it (left-arm view ends up grazing/self-occluded). Lateral target on the head **elbow** `arm_head_4_link`: `y_elbow → LEAN_FRACTION·y_hand`. Elbow chosen because joints 5-7 are downstream of it → its Jacobian columns there are structurally zero, so the wrist *cannot* absorb the task. The single `LOCAL_WORLD_ALIGNED` row is damped-inverted into a joint velocity and added to the posture channel, which `g` pre-multiplies by `JOINT_WEIGHTS` (cost = `0.5‖dq − dq_target‖²_W`) so the proximal weights cancel — unlike the roll/approach terms, whose un-compensated `g += w·Jᵀv` authority is divided by each joint's weight.
...
**Full simulation launch sequence** (robot side, then teleoperation side). `simulation_bringup.launch.py` (added, replaces the old 4-terminal Gazebo/controllers/RViz sequence) starts Gazebo + robot spawn, TSID default controllers (`CONTROLLER_DELAY_S=10s` after spawn, controller_manager isn't up before that), the mobile-base teleop, and TWO RViz stations (`study_operator.rviz`, `study_overview.rviz` — only the operator one shows the head camera) at `RVIZ_DELAY_S=14s`. It does NOT start the QP/shared-autonomy nodes or `study_recorder.py` — those stay separate terminals so they can be restarted independently without relaunching Gazebo:
```bash
cd ~/exchange/ros2-ws
