...
**Hardware-trial figures (thesis §4.1)**: `scripts/analysis/export_offline_bag.py <bag_dir>` reads an `offline_plotter.py` trial bag (or any sqlite3 rosbag2 of the QP stack) through sqlite3 + `rclpy.serialization` — no `rosbag2_py` needed — into one `.mat` (`series.<topic>.{t,data}`, JointState as `names/position/velocity`, `derived.gov`/`derived.traj` = offline_plotter's governor and tracking derivations replayed event-ordered). t0 = trigger rising edge, else falling edge − the `t_off` parsed from the trial's `trial_summary.txt` (the recorder is spawned from inside the trigger callback, so the rising edge is routinely missing from the bag), else first message. `scripts/analysis/matlab/fig_thesis_hw.m(mat, out_dir, 'panels', …)` draws one `hw_<key>.pdf` per panel (registry-based: λ_cbf, λ_jl, margin, d_min, d_safe, w_δ, governor raw/governed ×4, q/q̇ per arm, …; absent topics are skipped, so the same call serves a teleoperation bag), in `fig_thesis_panel` style; LaTeX lays them out as lettered subfigures. The raw bag used is in `thesis/figures/results/Home-> Cross2/` (untracked, 185 MB). `fig_thesis_hw.m`'s broken-axis λ_cbf panel and its `gov_ep`/`gov_theta` reference lines are trial-agnostic: the axis-break range is passed via `'lambda_break', [bottom_ceil top_floor top_ceil]` (chosen per-trial from where the data actually has a gap — home-to-crossed: `[180 500 600]`, teleop: `[500 1000 1100]`), and the governor ceilings (`0.30` m, `1.0` rad) are fixed real-hw-branch config constants, not read off each trial's own data (a trial whose governor barely saturates would otherwise draw a "ceiling" at its own coincidental peak). §4.1.2 reuses this pipeline on `thesis/figures/results/Real_Teleop_10_CFB_SUCCESS/` (untracked bag + `Teleop1-4.jpeg` + `PoseEstimation.png`, a *different* trial's perception-debug screenshot the user supplied as illustrative reference only): a CFB teleoperated bimanual grasp with one failed side-grasp (12.0 s alignment timeout), one successful top-grasp, and a failed second side-grasp whose retreat stalled and oscillated into an emergency stop (deadman release) — timeline cross-validated from `/shared_autonomy/{gripper_cmd,grasp_margin,grasp_active,active_arm}` against `grasp_state_machine.py`'s own timeout constants. That bag has **no** `/perceived_world/snapshot` topic (this trial's `world_source` defaulted to `'yaml'`, not `'perceived'`), so the cylinder radius/height/pose shown in `PoseEstimation.png` cannot be reconstructed from it — a real per-trial perception/cylinder figure needs either a bag recorded with `world_source:=perceived` or the declared YAML scenario's dimensions read as "configured", not "estimated".
