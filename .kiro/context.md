...
> **The same frame principle applies to `main_head.py`.** Its geometric RGB-D pipeline transforms the depth cloud via TF using the *depth* optical frame; the Gazebo render-vs-tagged-frame offset (#1) affects that frame too, so apply the link-position correction to the depth frame before judging the geometric pipeline's accuracy.
