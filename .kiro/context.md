...
Other fairness unifications live on the haptic side (see `haption_teleoperation` context §3.0): the clutch sync spring (`30 / 0.9`), authority cap (`10 N / 1 Nm`), constant global damping (`0.7 / 0.1`), and the clutch orientation-alignment torque (`K_align=10`, now a sync effect present in every clutch cell) are identical across all four clutch cells; every B=1 cell renders the same blend-telemetry window from `/shared_autonomy/blend_debug`. **Accepted residual asymmetry (D3):** during autonomous grasp execution the CLUTCH force managers drag the handle to follow the EE, while the JOYSTICK managers do not subscribe to `grasp_active` and render nothing extra — so the autonomous grasp is felt only in clutch. Left as-is for now, documented for the paper.
...
- **Stand-off**: depth error `Z − TARGET_DISTANCE` (`TARGET_DISTANCE = 0.8 m`, overridable via the `target_distance` ROS param) → camera translates to hold 80 cm.
...
**Soft approach-axis alignment (viewpoint/orbit preference, `ENABLE_APPROACH_ALIGN` / `-p approach_align`)**: optionally aligns the optical axis (+Z) with the active gripper's approach axis (tool-frame +X) so the head "looks along where the hand is reaching" (a top grasp → top-down view). Since centering already owns the optical-axis *direction*, this is NOT an orientation servo — it's a position/orbit bias: the aligned pose is the point on the stand-off sphere lying on the approach line, `p_cam_des = p_hand - TARGET_DISTANCE · x_gripper`. A low-weight LS term on the camera's *linear* velocity toward that point (`H[:7,:7] += w·JlinᵀJlin`, `g[:7] += w·Jlinᵀ·v_des_cam`, `Jlin = J_cam[:3,:]`, `v_des_cam = R_camᵀ·K_APPROACH_ALIGN·(p_cam_des − p_cam)` clamped) orbits the head to the viewpoint; centering re-points and stand-off holds the radius, so it resolves in their null space. Centering-gated (fades to zero off-axis, so it never fights acquisition/PBVS) and modest-weighted — subordinate, never overriding, same spirit as the roll term.
...
Alternative head-perception path (sibling of `main_head.py`, same look-at control reused verbatim: `HeadKinematics` + `LookAtController` + controller switching). It localizes ONE fiducial of known geometry and reconstructs every scene object from a known rigid transform relative to it:
...
**Correct camera TF setup — THIS is what makes head-camera perception accurate.** There is no inherent "head-camera vision bias"; the historical centimetre-level errors were entirely a frame/extrinsic setup problem. With the four points below satisfied, reconstruction reaches **~2 mm** in sim (the ViSP sub-pixel noise floor, essentially exact — this is the first head-camera setup that achieves it). Any one of them wrong yields a large *systematic* offset that looks like a "bias":

1. **Optical CENTRE frame** (`cfg.APRILTAG_OPTICAL_CENTER_FRAME` / `-p optical_center_frame`). ViSP's `c_M_tag` is in optical convention (X-right, Y-down, Z-forward). Take the camera *orientation* from the image's optical frame but the *position* from the true optical centre — these are NOT the same frame in sim. **The Gazebo RealSense plugin renders from the camera-link origin** yet tags the image as `..._color_optical_frame`, which the URDF places ~2.5 cm away (measured `camera_link→color_optical_frame` = `[-0.011,-0.009,-0.021]`). So in sim the position MUST come from `gripper_head_camera_rgbd_link`; using the optical frame's position offsets every object by exactly that translation. On **real** RealSense hardware the driver's color-optical-frame origin IS the sensor, so set this equal to `APRILTAG_CAMERA_OPTICAL_FRAME` (no correction).
2. **Tag-frame orientation in world** (`cfg.APRILTAG_RPY_WORLD`) must match the actual rendered/printed tag. ViSP default (`align_z=false`) frame: Z out of the tag toward the camera, X=image-right, Y=image-up (`cMo = Rot_x(180°)` fronto-parallel). In this sim Gazebo's OGRE box-face UV maps image-right → world −Y, giving tag yaw **−90°** (X_tag→−Y, Y_tag→+X, Z_tag→+Z) ⇒ `APRILTAG_RPY_WORLD = [0,0,−π/2]`. A wrong yaw rotates the whole reconstruction rigidly about the tag centre. On the real robot, read the tag-frame yaw off the live `tracker_apriltag` TF and set it.
3. **Subscriber QoS** must match the `visp_apriltag` publisher (BEST_EFFORT + TRANSIENT_LOCAL); a default RELIABLE subscriber silently receives nothing.
4. **ViSP pose method** `best_residual_virtual_vs` (config default, passed to the `visp_apriltag` node) — markedly lower bias than `homography_virtual_vs` at this tag size/range.

Diagnose all of the above with the read-only `[TAG-DIAG]` block (`-p tag_diag`), which logs the estimated-vs-known tag pose, the raw ViSP `c_M_tag`, and the TF extrinsic (comparison-only; the known pose is never fed into the reconstruction).

> **The same frame principle is now applied to `main_head.py`** (`cfg.DEPTH_OPTICAL_CENTER_FRAME` / `-p depth_center_frame`). With this correction + scan disabled (`ENABLE_SCAN=False`) the geometric RGB-D pipeline also reaches **<1 cm** accuracy in sim (slowly converges to the true pose via the object-level EMA tracker). Scan is disabled because the depth pipeline transforms 30k+ points per frame via TF: even a few-ms TF lag from `robot_state_publisher` during head motion rotates the entire cloud by a few degrees, shifting detected centroids by ~5–10 cm. With a still head, TF converges and the cloud transform is exact. `head_april_main` can safely scan because it transforms a single point (the tag pose), where the same TF lag causes <2 mm lateral error. Control gains are `MAX_HEAD_VELOCITY=0.10`, `LOOKAT_LAMBDA=1.0` (gentle, matching head_april).
...
Diagnosed via `/qp_debug/loop_timing`'s per-phase breakdown (§10.6): on real hardware `compute_softmin_jacobian` (CBF aggregation) was the single largest phase, and `qp_visualizer_tutorial.py`'s collision-witness marker independently re-scanned the same `cdata.distanceResults` `compute_softmin_jacobian` had just scanned.
...
6. **Control-loop timing is measured, not assumed, on real hardware.** `main_qp_controller.py` uses a fixed nominal `dt = 1/CONTROL_FREQ_DEFAULT` in the CLF/CBF/governor math (never the measured wall-clock gap between ticks), so scheduling jitter is invisible to that math — same-host sim (loopback DDS discovery, uncontended CPU) hides this; the real robot does not. `/qp_debug/loop_timing` (`Float64MultiArray`, published every tick, NOT downsampled: `[tick_dt_ms, kin_ms, cbf_ms, gov_ms, solve_ms, telemetry_ms, misc_ms]`) breaks the tick into phases — kinematics/FK refresh, SoftMin CBF aggregation, reference governor + task error, QP build+solve, downstream telemetry/command/viz publish, and a `misc_ms` residual (unbracketed code + any genuine OS scheduling wait) — so a real-hardware timing/E-stop investigation can pin the cost to a specific phase via `loop_timing_monitor.py` (§11) instead of guessing. In practice this diagnosed a real run: the controller process sat near 100% CPU on a single core (`ps`), i.e. genuinely compute-bound rather than OS-scheduling-starved (a starved process would show LOWER %CPU, not higher) — check the process's own `ps`/`chrt -p PID`/`taskset -pc PID` first, since Python's GIL means extra idle cores never help a single-threaded bottleneck, and only the phase breakdown tells you which part of the tick to actually optimize (or, as a last resort, whether a frequency drop + full gain retune is warranted).
