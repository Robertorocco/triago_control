...
**Waypoint clearance rule (from the first `s_curve_*` cut, 2026-07-12)**: the CBF enforces `h = distance − CAPSULE_RADIUS(0.06) − D_SAFE_BASE(0.015)`, not distance-to-obstacle-surface — budget any hand-authored `mode: path` waypoints against `h`. The first cut (z=0.775 cylinder mid-height, ±0.08m bulge) was infeasible the entire sweep (h≈−0.03 at the cylinders, ≈0 over the table) and produced `max_lambda_cbf=44.9` + heavy qdot oscillation; a controlled A/B with only the waypoints changed (to h≈+0.08) collapsed every symptom — proving the oscillation mechanism was QP active-set chatter under a reference persistently pinning the CBF, not a frequency artifact of the 300→150Hz / 100→50Hz changes. The infeasible cut is preserved verbatim as the `s_curve_right_infeasible_regression` acceptance preset (do not "fix" its numbers — it exists to stress the controller). Ops note: an applied fix to this hand-edited YAML was silently lost once to a stale editor buffer — if a config change doesn't move the results at all, diff the on-disk file first (`s_curve_diagnostic.py` prints the commanded reference for exactly this check).

**qdot-oscillation resolution — measured-anchor rate damping (2026-07-12, final)**: `ENABLE_RATE_DAMPING=True` + `RATE_DAMPING_VS_MEASURED=True` + `RATE_WEIGHT=50` adds `RATE_WEIGHT·‖q̇ − q̇_measured‖²` (ARM joints only) to the QP cost — tick-to-tick memory anchored to the arm's real state. Why it works: the ripple lived in the right wrist (joints 5/7 at ~50% of their own mean command, per `s_curve_diagnostic.py`'s per-joint table) because under 25:1 position:orientation task weights + high slack those DOFs are near-null-space, resolved purely by a cost that previously had no memory. Weight ladder on the infeasible-regression stress case: 10→26.6% cmd ripple, 25→14.0%, **50 (adopted)→8.3%**, with `qdot_measured` ripple 0.0058→0.0011 rad/s (pipeline noise floor ≈0.0005), near-obstacle ripple now BELOW far-field (was 2×), and `min_observed_distance` improving monotonically with weight. Full validation matrix at weight 50: free space 5.9%→0.7%, safe S-curve 5.4%→1.7% (all 14 joints ≤3.9%), infeasible regression 50.9%→8.3%. Trade-off: more caution near obstacles (λ peaks 3×, governor 52%→92%, safe-curve completion ~21s→~37s) — drop to 25 for a snappier compromise at 14% worst-case ripple. Real-hardware validation open.

**Rejected along the way (mechanisms understood — don't retry)**: commanded-anchor rate damping (`RATE_DAMPING_VS_MEASURED=False`, kept as a toggle) — anchoring on the QP's own previous output is a self-referential low-pass in the loop's forward path; command got smoother, real motion got ~2× worse. Hard slew-rate box on q̇ (code removed) — a per-joint box can't relieve the CBF row (λ_cbf reflects a standing geometric conflict, not a rate problem) and structurally breaks QP feasibility when the CBF row's required direction swings faster than the box allows. `DYNAMIC_SLACK_WEIGHT=False` — no ripple change, tracking measurably worse (orientation sacrificed first under 25:1 weights). `DAMP` 12→15 — no ripple change, worse tracking. Posture field exonerated by data: the worst-ripple joints sat mid-range (p≈0.06) while the joint deepest into the field (p≈−0.79) was silent.
...
- Rate damping toward the MEASURED joint velocity (`RATE_WEIGHT=50`, arm joints only — see the oscillation digest in §2).
...
`cfg.ENABLE_RATE_DAMPING` is now default **True** with `RATE_DAMPING_VS_MEASURED=True` — the 2026-07-11 rejection recorded here applied only to the commanded-output anchor (`dq_prev = last_dq_safe`, a self-referential in-loop filter). Anchoring on the MEASURED velocity instead resolved the qdot oscillation outright — full story in the oscillation digest, §2.
