...
**ViSP tag-frame convention** (verified empirically against the installed ViSP): default `align_z=false` gives `cMo = Rot_x(180°)` for a fronto-parallel view → Z out of the tag toward the camera, X=image-right, Y=image-up. The tag frame orientation in `base_footprint` is captured by `cfg.APRILTAG_RPY_WORLD` and MUST match the actual rendered/printed tag. **Measured in sim**: Gazebo's OGRE box-face UV maps the tag image-right to world −Y (not +X), so the real tag-frame is yaw −90° (X_tag→−Y, Y_tag→+X, Z_tag→+Z) → `cfg.APRILTAG_RPY_WORLD = [0,0,−π/2]`. Getting this wrong rotates the reconstructed objects 90° about the tag centre (was a ~40 cm error with the initial `[0,0,0]` guess); the correct yaw makes the reconstruction exact. On the real robot, read the tag-frame yaw off the `tracker_apriltag` TF and set this accordingly. The subscriber QoS must match the visp_apriltag publisher (BEST_EFFORT + TRANSIENT_LOCAL) — a default RELIABLE sub receives nothing.

**Optical-CENTRE frame (sim vs real, `cfg.APRILTAG_OPTICAL_CENTER_FRAME` / `-p optical_center_frame`)**: ViSP's `c_M_tag` is in optical convention; the node takes the camera *orientation* from the image's optical frame but the *position* from this centre frame. In **Gazebo** the RealSense plugin renders the image from the **camera-link origin** yet tags it as `..._color_optical_frame`, which the URDF offsets from the link by ~2.5 cm (`camera_link→color_optical_frame` translation `[-0.011,-0.009,-0.021]`) — so ViSP's pose is really relative to the link, and using the offset optical-frame position produced a constant **~2.7 cm** bias on every reconstructed object (verified: the bias equals that offset rotated into the optical frame). Fix = use the link position + optical orientation (sim default `gripper_head_camera_rgbd_link`). On **real** RealSense hardware the driver's color optical frame origin IS the true sensor, so set this equal to `APRILTAG_CAMERA_OPTICAL_FRAME` (no correction). A read-only `[TAG-DIAG]` block (`-p tag_diag`) logs the estimated-vs-known tag pose, the raw ViSP `c_M_tag`, and the TF extrinsic to localize such biases (comparison-only; never fed into the reconstruction).
