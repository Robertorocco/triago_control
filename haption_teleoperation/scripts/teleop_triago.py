...
class ForwardTeleopIntegrator(Node):
    def __init__(self):
        super().__init__('forward_teleop_integrator')
        ...
        self.get_logger().info("Forward Teleop Integrator started. Waiting for initial TRIAGo pose...")
    ...
    def integration_loop(self):
        ...
        # Left-multiplication applies the rotation in the global (base) frame
        ...
        cmd_msg.data = [
            ...
            float(self.v_cmd[0]),   float(self.v_cmd[1]),   float(self.v_cmd[2]),   # 6:9 Linear Vel
            float(self.w_cmd[0]),   float(self.w_cmd[1]),   float(self.w_cmd[2]),   # 9:12 Angular Vel
